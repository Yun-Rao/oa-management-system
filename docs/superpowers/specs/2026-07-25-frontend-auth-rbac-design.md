...
- [ ] 手动冒烟(后端 :8000 + dev server 代理):错误密码登录显示"邮箱或密码错误";admin 登录成功跳首页
- [ ] admin 可见"用户管理"菜单,进入列表可搜索/翻页;新建用户后出现在列表
- [ ] 可编辑用户姓名、禁用后该用户登录被 401 拒绝;分配角色后 `GET /auth/me` 权限点变化生效
- [ ] 非 admin(无 user:list)登录看不到"用户管理"菜单,直访 /users 跳回首页
- [ ] 顶栏"修改密码"改密成功后,旧密码登录失败、新密码可登录
