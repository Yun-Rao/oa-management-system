...
- [x] `npm test` 全绿(地基 11 + 本期新增)
- [x] `tsc --noEmit` 零错误,`vite build` 成功

**浏览器实测(由执行 Agent 使用 chrome-devtools 驱动真实浏览器完成,非人工点检;前置:后端 :8000 + dev server :5173 代理):**

- [x] 错误密码登录:页面显示"邮箱或密码错误",不跳转
- [x] admin 登录成功:跳转首页,顶栏显示用户名,菜单含"用户管理"
- [x] 用户管理:列表渲染;关键字搜索过滤;翻页;新建用户后出现在列表
- [x] 编辑用户姓名生效;禁用某用户后,该用户登录被 401 拒绝;为其分配角色后该用户 `GET /auth/me` 权限点变化
- [x] 非 admin(无 user:list)登录:菜单无"用户管理",直访 /users 跳回首页
- [x] 顶栏"修改密码":改密成功后旧密码登录失败、新密码可登录
- [x] 每个场景截图存档至 `.superpowers/sdd/`(或执行会话指定的验收目录),作为验收证据
