...
- [ ] 员工可提交含 1~5 个附件凭证的报销申请,金额/类型/说明齐全
- [ ] 金额 ≤ 阈值主管审批即通过;> 阈值主管通过后流转二级,页面清晰展示当前第几级、审批人是谁
- [ ] 任一级驳回流程终止,状态「已驳回」且驳回原因可见;撤回在审批中可用
- [ ] 我的申请/待我审批/全部记录三视图按权限可见,全部记录可按部门/状态/类型/时间筛选
- [ ] 附件凭证可在详情中鉴权下载
- [ ] 消息中心点击报销通知可跳转 `/expenses` 并自动打开对应详情
