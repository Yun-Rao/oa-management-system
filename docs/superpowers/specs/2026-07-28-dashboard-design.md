...
- [x] `GET /api/v1/dashboard` 按 §3 契约返回三段统计,month 缺省本月、非法 422
- [x] 请假统计口径符合 §4.1(approved、跨月切分、人次跨月各计)
- [x] 报销统计口径符合 §4.2(approved、创建月归属)
- [x] 审批时效口径符合 §4.3(终态完成月、含驳回不含撤回、不拆级别)
- [x] Admin 全量、Manager 仅本部门、无权限 403、未登录 401
- [x] seed 重跑幂等补齐两个新权限点;全量 pytest 绿(既有 247 + 新增,无回归)
