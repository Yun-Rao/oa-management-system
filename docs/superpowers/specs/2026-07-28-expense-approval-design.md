...
- [x] 金额 ≤ 阈值仅需主管审批即通过;> 阈值需主管通过后再经 HR/Admin 二级审批
- [x] 任一级驳回流程终止,状态"已驳回";全部通过状态"已通过"
- [x] 阈值可配置(环境变量,不写死在代码里)
- [x] 二级审批场景下,可清晰区分当前处于第几级、审批人是谁
- [x] 报销申请必含 1~5 张附件凭证,附件可鉴权下载
- [x] 各触发点通知正确送达(提交→主管;转入二级→扇出权限池;通过/驳回→申请人;撤回不通知)
